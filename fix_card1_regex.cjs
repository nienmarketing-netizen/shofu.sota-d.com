...
// We use string manipulation to find the exact indices
const startTag = "{/* 1. Clinical Evidence JADA */}";
const endTag = "{/* 2. Molecular Mechanism */}";

const startIndex = content.indexOf(startTag);
const endIndex = content.indexOf(endTag);

if (startIndex !== -1 && endIndex !== -1) {
    const before = content.substring(0, startIndex + startTag.length + 1);
    const after = content.substring(endIndex);
    
    const finalContent = before + "\n            " + newCard1 + "\n\n            " + after;
    fs.writeFileSync(file_path, finalContent);
    console.log("Card 1 successfully updated using index search.");
} else {
