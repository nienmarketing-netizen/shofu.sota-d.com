...
content = content.replace(
  /\{\s*id:\s*1,\s*title:\s*"Phục hình Răng thẩm mỹ"[\s\S]*?description:\s*"Hiệu ứng quang học xuất sắc, độ bóng tự nhiên duy trì lâu dài."\s*\}/,
  newCase1
);

fs.writeFileSync('src/components/shofu/ShofuCaseStudies.tsx', content);
console.log("Updated Case 1 verbatim (fixed!)");
